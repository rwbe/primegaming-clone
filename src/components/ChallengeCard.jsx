...
import ExternalLinkIcon from '../assets/svg/Link.svg';

const ChallengeCard = ({ title, subtitle, img }) => {
  // Função para resumir o subtítulo
  const truncateSubtitle = (text, maxLength) => {
    if (typeof text !== 'string' || text.length <= maxLength) return text;
    const truncatedText = text.substr(0, maxLength);
    const lastSpace = truncatedText.lastIndexOf(' ');
    return lastSpace === -1 ? truncatedText : truncatedText.substr(0, lastSpace) + '...';
  };

  return (
    <div className='min-w-[215px] max-sm:min-w-[125px] max-w-[348px] mb-[18px] max-md:min-h-[275px] min-h-[289px] bg-[#071936] rounded-md overflow-hidden relative'>
      ...
      <div className='w-full h-[180px] max-md:h-[85px] overflow-hidden'>
        <img src={img} alt="" className='w-auto h-auto object-cover hover:scale-105 transition-all' />
      </div>

      <div className='flex flex-col p-3 pt-2 h-auto'>
        <h1 className='text-white font-bold text-base mb-1'>{title}</h1>
        <p className='text-cardSubtitle font-medium text-sm mb-1 md:mb-3'>{truncateSubtitle(subtitle, 75)}</p>
        <button
          className='text-txtPrimeBlue text-sm min-h-[38px] mt-2 font-medium px-3 py-[3px] rounded-md flex items-center justify-center bg-primeBlue hover:bg-primeBlueHover'
          style={{ width: '80px' }}>
          Jogar
          <span className="mx-1"></span>
          <img src={ExternalLinkIcon} alt="Link Icon" className="w-4 h-4 mt-1" />
        </button>
      </div>
    </div>
  );
